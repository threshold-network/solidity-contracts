...
==== `requestAuthorizationDecrease(address operator, address application) external onlyAuthorizerOf(operator)`

Requests decrease of the authorization for the given operator on the given 
application by all authorized amount. Calls `authorizationDecreaseRequested(address operator, uint256 amount)`
on the given application. It may not change the authorized amount immediatelly. When
it happens depends on the application. Can only be called by the given operator’s 
...
Requests decrease of the all authorizations for the given operator on the all 
...
==== `topUp(address operator, uint96 amount) external`
...
==== `unstakeT(address operator, uint96 amount) external`
...
==== `unstakeNu(address operator, uint96 amount) external`
